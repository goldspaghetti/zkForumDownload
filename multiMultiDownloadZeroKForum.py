...
#for each forum post
def pageDownload(linkNum, title, postNum, numbers):
    ...
    try:
        ...
        raise Exception('Error occured at download: ' + postLink)
...
def downloadZeroK(linkNum):
    ...
    for numbers in range(pageAmount):
        pageThread = threading.Thread(target=pageDownload, args=[linkNum, title, postNum, numbers])
...
for i in range(0, len(linkElem)-1):
    ...
    print('amount of threads: ' + str(len(downloadThread)))
